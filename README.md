...
**3-2. Supabase 대시보드에서 Clerk 인증 제공자 추가**
...
통합이 성공적으로 설정되면 Clerk 세션 토큰이 Supabase 요청에 자동으로 포함됩니다.
...
- `/tasks-example`: Clerk 공식 문서 예시 기반 Tasks 페이지 (RLS 정책 테스트용)
- `/instruments`: Supabase 공식 문서 예시 기반 Instruments 페이지 (공개 데이터 조회 예시)
...
- **기본 설정**: `app/layout.tsx`의 `ClerkProvider`에서 커스텀 한국어 로컬라이제이션 사용
- **커스텀 메시지**: `lib/clerk/localization.ts`에서 에러 메시지 등 커스터마이징 가능
- **Tailwind CSS 4 호환**: `cssLayerName: "clerk"` 설정으로 Tailwind CSS 4와 호환

**📖 상세 가이드**: [CLERK_LOCALIZATION.md](docs/CLERK_LOCALIZATION.md) 참고
...
**RLS 정책 예시**: `supabase/migrations/20250103000000_add_rls_policies_example.sql` 파일을 참고하세요.
...
**📖 상세 가이드**: [CLERK_SUPABASE_INTEGRATION.md](docs/CLERK_SUPABASE_INTEGRATION.md)의 RLS 정책 설정 섹션 참고
...
- [Supabase Quickstart 가이드](docs/SUPABASE_QUICKSTART.md) - Supabase 공식 문서 기반 가이드
- [Clerk 한국어 로컬라이제이션 가이드](docs/CLERK_LOCALIZATION.md) - Clerk 컴포넌트 한국어 설정
- [프로젝트 구조 가이드](docs/DIR.md) - 디렉토리 구조 설명
- [할 일 목록](docs/TODO.md) - 개발 진행 상황
